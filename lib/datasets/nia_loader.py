...
class nia (data.Dataset):
  def __init__ (self, opts, image_set='train', dataset_option='normal', batch_size=1, use_region=False):
    #print ('[+] nia_loader __init__ called')

    self.opts = opts
    self.use_region = use_region
    self._name = 'nia_' + dataset_option + '_' + image_set
    ...
    self.voc_sign = {'start': self.word2idx[self.start_token], 'null': self.word2idx[self.unknown_token], 'end': self.word2idx[self.end_token]}
    ...
    ann_file_name = {
      'nia_normal_train': 'train.json', 
      'nia_normal_test' : 'test.json', 
      'nia_small_train' : 'train_small.json',
      'nia_small_test'  : 'test_small.json', 
      'nia_fat_train'   : 'train_fat.json', 
      'nia_fat_test'    : 'test_fat.json'
    }

    ann_file_path = osp.join (annotation_dir, ann_file_name[self.name])
    #print ('hyhwang', ann_file_path)
  ...
  def __getitem__ (self, index):
    ...
    if self._image_set == 'train': # calculate the RPN target
      item['rpn_targets']['object'] = anchor_target_layer (item['visual'], gt_boxes_object, item['image_info'],
                self._feat_stride, self._rpn_opts['object'], mappings = self._rpn_opts['mappings'])
  ...
  def __len__ (self):
    #print ('__len__', self.annotations)
    return len (self.annotations)


  @property
  def voc_size (self):
    #print ('voc_size', self.idx2word)
  ...
  def untokenize_single_sentence (self, sentence):
    #print ('untokenize_single_sentence')
    word_sentence = []
    for idx in sentence:
  ...
  def num_object_classes (self):
    #print ('num_object_classes', len(self._object_classes))
    return len (self._object_classes)

  @property
  def num_predicate_classes (self):
    #print ('num_predicate_classes', len(self._predicate_classes))
    return len (self._predicate_classes)

  @property
  def object_classes (self):
    #print ('object_classes', self._object_classes)
    return self._object_classes

  @property
  def predicate_classes (self):
    #print ('predicate_classes', self._predicate_classes)
