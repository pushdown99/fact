...
class Factorizable_network (nn.Module):
    ...
    def __init__ (self, trainset, opts = None):
        ...
        with open (opts['rpn_opts'], 'r') as f:
            self.rpn_opts = yaml.full_load (f)
    ...
    def forward (self, im_data, im_info, gt_objects=None, gt_relationships=None, rpn_anchor_targets_obj=None):
        ...
        if self.learnable_nms:
            ...
            if duplicate_weights.data.sum () == 0:
                ...
                mask = mask.ge (1)  # hyhwang torch.uint8 => torch.bool
                #print (mask)
                ...
                loss_nms = F.binary_cross_entropy (selected_prob, duplicate_labels,
                                    ...
                                    reduction='sum') / (duplicate_weights.data.sum () + 1e-10)
    ...
    def evaluate (self, im_data, im_info, gt_objects, gt_relationships, thr=0.2, nms=-1., triplet_nms=-1., 
        ...
        else:            object_result, predicate_result = self.forward_eval (im_data, im_info,)
        ...
        obj_boxes, obj_scores, obj_cls, subject_inds, object_inds, subject_boxes, object_boxes, predicate_inds, sub_assignment, obj_assignment, total_score = \
                interpret_relationships (cls_prob_object, bbox_object, object_rois, cls_prob_predicate, mat_phrase, im_info,
                            nms=nms, top_N=max (top_Ns), use_gt_boxes=use_gt_boxes, triplet_nms=triplet_nms, reranked_score=reranked_score)
